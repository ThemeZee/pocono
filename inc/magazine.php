...
*/
function pocono_magazine_widgets() {
	...
	elseif ( is_page_template( 'template-magazine.php' ) && current_user_can( 'edit_theme_options' ) ) :
		...
		esc_html_e( 'Please go to Customize &#8594; Widgets and add at least one widget to the Magazine Homepage widget area.', 'pocono' );
		echo '</p>';
...
if ( ! function_exists( 'pocono_magazine_widget_title' ) ) :
	/**
	 ...
	 */
	function pocono_magazine_widget_title( $widget_title, $category_id ) {

		// Check if widget shows a specific category.
		if ( $category_id > 0 ) {

			// Set URL and Title for Category.
			$category_title = sprintf( esc_html__( 'View all posts from category %s', 'pocono' ), get_cat_name( $category_id ) );
