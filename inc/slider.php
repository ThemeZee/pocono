...
function pocono_slider_scripts() {
	...
	if ( true === $theme_options['slider_blog'] or is_page_template( 'template-slider.php' ) ) :
		...
		wp_enqueue_script( 'pocono-slider', get_template_directory_uri() . '/assets/js/slider.js', array( 'jquery-flexslider' ), '20170421' );

		// Register and enqueue slider CSS.
		wp_enqueue_style( 'pocono-slider', get_template_directory_uri() . '/assets/css/flexslider.css', array(), '20170421' );
...
if ( ! function_exists( 'pocono_slider_image' ) ) :
	/**
	 ...
	 */
	function pocono_slider_image( $size = 'post-thumbnail', $attr = array() ) {
...
function pocono_slider_options() {
	...
	wp_localize_script( 'pocono-slider', 'pocono_slider_params', $params );
